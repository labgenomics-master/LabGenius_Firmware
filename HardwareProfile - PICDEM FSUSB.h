...
/** ADC for temperature sensor*********************************************/
	...
	#define Sensor_Photodiode	ADC_CH_00
	#define Sensor_Chamber		ADC_CH_01
	#define Sensor_Heatsink		ADC_CH_04
	...
	#define Init_PWM_MODE()	{InitPWMPort();\
							 T2CONbits.TMR2ON = 0;\
							 PR2 = 0xFF;\
							 CCP1CON =  PxM_SINGLE_OUTPUT | CCPxM_PWM_ACTIVE_ACH_BDH;\
							 T2CONbits.TMR2ON = 1;}
	...
	/**fan ************************************************************/
	#define InitFans()		TRISCbits.TRISC6 = OUTPUT_PIN; LATCbits.LATC6=0;
	#define Chamber_Fan 	LATCbits.LATC6
	
 	#define Fan_ON()         Chamber_Fan = 1;  //n-mos:1 p-mos:0
    #define Fan_OFF()        Chamber_Fan = 0;  //n-mos:0 p-mos:1
